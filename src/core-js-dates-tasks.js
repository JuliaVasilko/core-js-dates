...
function getCountDaysInMonth(month, year) {
  let daysInMonth = 28;
  let curentMonth = month - 1;

  while (curentMonth === month - 1) {
    daysInMonth += 1;
    const newDate = new Date(year, curentMonth);
...
function isDateInPeriod(date, period) {
  ...
  return periodStart <= dateToCheck && dateToCheck <= periodEnd;
}
...
function formatDate(date) {
  ...
  const second = new Date(date).getUTCSeconds();
  return `${month + 1}/${day}/${year}, ${hour % 12 || 12}:${minute > 10 ? minute : `0${minute}`}:${second > 10 ? second : `0${second}`} ${hour >= 12 ? 'PM' : 'AM'}`;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(getFormatedDate(currentDate));
...
function isLeapYear(date) {
  ...
  const secCond = year % 100 !== 0 && year % 400 === 0;
  return dividedByfour || secCond;
}
